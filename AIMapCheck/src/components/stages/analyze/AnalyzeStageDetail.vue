    ...
    <div class="panel-content">
      ...
      <div class="panel-body">
        <div class="status-section">
          <div v-if="stageData" class="status-badge" :class="stageData.status">
            {{ getStatusText(stageData.status) }}
          </div>
          <div v-if="stageData && stageData.progress !== undefined" class="progress-bar">
        ...
        <div class="description-section">
          <p>在此阶段，系统将对上传的地图进行智能解析。解析过程包括：</p>
          <ul>
            <li>图层分析：识别和分离地图中的不同图层</li>
            <li>要素提取：提取地图中的关键地理要素</li>
            <li>文字识别：识别地图中的文字信息</li>
            <li>符号识别：识别地图中的专业符号</li>
            <li>空间分析：分析地理要素的空间关系</li>
          </ul>
          <p>解析过程可能需要几分钟时间，请耐心等待。解析完成后，系统会自动进入下一阶段。</p>
        </div>

        <div class="actions">
          <button 
            class="start-btn" 
            :disabled="!stageData || stageData.status === 'completed' || stageData.status === 'in_progress'"
...
const handleStartAnalyze = async () => {
  try {
    const response = await axios.post(
      `http://8.148.68.206:5000/api/tasks/${taskId}/analyze/start`
    )
